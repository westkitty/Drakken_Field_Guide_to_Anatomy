...
if (!await page.evaluate(() => document.querySelector('#registry-drawer')?.contains(document.activeElement) ?? false)) throw new Error('Registry focus escaped its modal drawer.');
...
const recordState = await page.evaluate(() => {
  ...
  return {
    ...
    exportPosition: getComputedStyle(footer).position,
    footerOverlap: Math.max(0, detailRect.bottom - footerRect.top),
  };
});
...
if (recordState.exportPosition !== 'relative') throw new Error(`Export footer is not in normal flow: ${recordState.exportPosition}.`);
if (recordState.footerOverlap > 1) throw new Error(`Export footer overlaps annotation detail by ${recordState.footerOverlap}px.`);
...
const mobileTools = await page.evaluate(() => {
  ...
  const toolGroups = [...panel.querySelectorAll('.tool-cluster')];
  const layerGrid = panel.querySelector('.layer-grid');
  return {
    left: rect.left,
    right: rect.right,
    top: rect.top,
    ...
    horizontalOverflow: panel.scrollWidth > panel.clientWidth + 1,
    internalGroupOverflow: toolGroups.some((group) => group.scrollWidth > group.clientWidth + 1),
    layerOverflow: layerGrid.scrollWidth > layerGrid.clientWidth + 1,
    layerColumns: getComputedStyle(layerGrid).gridTemplateColumns.split(' ').filter(Boolean).length,
...
if (mobileTools.horizontalOverflow || mobileTools.internalGroupOverflow || mobileTools.layerOverflow) throw new Error(`Mobile tools contain clipped horizontal content: ${JSON.stringify(mobileTools)}.`);
if (mobileTools.layerColumns !== 2) throw new Error(`Expected two mobile anatomy columns, found ${mobileTools.layerColumns}.`);
...
if (narrowRecordState.tabsPosition !== 'relative' || narrowRecordState.exportPosition !== 'relative') throw new Error(`Narrow record chrome did not use normal flow: ${JSON.stringify(narrowRecordState)}.`);
...
fs.writeFileSync(path.join(outputRoot, 'polish-browser-audit.md'), [
  ...
  '- In-flow export footer with zero annotation overlap: PASS',
  ...
  '- Mobile tools sheet containment, wrapped groups, and two-column layers: PASS',
