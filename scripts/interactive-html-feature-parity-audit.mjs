...
async function openDrawer(selector) {
  const drawerId = selector.slice(1);
  ...
  await page.waitForFunction((value) => {
    const panel = document.querySelector(value);
    if (!(panel instanceof HTMLElement)) return false;
    const style = getComputedStyle(panel);
    const rect = panel.getBoundingClientRect();
    const transitionSettled = style.transitionDuration.split(',').every((duration) => parseFloat(duration) === 0) || style.transform === 'none' || style.transform === 'matrix(1, 0, 0, 1, 0, 0)';
    return transitionSettled && rect.width > 0 && rect.height > 0;
...
async function closeDrawer(selector) {
  ...
  if (!closed) throw new Error(`No scrim available while closing ${selector}.`);
  await page.waitForFunction((value) => !document.querySelector(value)?.classList.contains('is-open'), { timeout: 10_000 }, selector);
  await delay(180);
}
...
await openDrawer('#registry-drawer');
...
await openDrawer('#record-drawer');
...
await openDrawer('#diagnostics-drawer');
...
const usableHeight = Math.max(responsive.innerHeight, responsive.visualHeight);
if (responsive.left < -1 || responsive.right > responsive.width + 1 || responsive.top < -1 || responsive.bottom > usableHeight + 1 || responsive.overflow) {
...
fs.writeFileSync(path.join(outputRoot, 'interactive-html-browser-audit.md'), [
  ...
  '- G/T/I/D and Escape shortcuts: PASS',
  ...
  '- Settled mobile containment: PASS',
  '- Remote runtime requests: 0',
  '- Actionable browser errors: 0',
  '',
].join('\n'));
console.log('Interactive HTML settled-state feature-parity audit passed.');
