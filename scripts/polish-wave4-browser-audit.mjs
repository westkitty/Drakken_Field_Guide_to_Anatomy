...
const restState = await page.evaluate(() => {
  ...
  return {
    ...
    toastRightGap: Number((window.innerWidth - toastRect.right).toFixed(2)),
    toastBottomGap: Number((window.innerHeight - toastRect.bottom).toFixed(2)),
  };
});
...
if (restState.toastRightGap < 10 || restState.toastBottomGap < 10 || restState.toastRightGap > 30 || restState.toastBottomGap > 30) {
  throw new Error(`Status toast is not anchored inside the lower-right safe area: ${JSON.stringify(restState)}.`);
}
...
const registryState = await page.evaluate(() => {
  ...
  const card = drawer.querySelector('.specimen-card[aria-current="true"]');
  ...
  return {
    ...
    drawerContained: drawer.scrollWidth <= drawer.clientWidth + 1,
  };
});
...
const toolsState = await page.evaluate(() => {
  ...
  const primary = drawer.querySelector('.primary-tools');
  const rectangles = [...primary.querySelectorAll('.tool-cluster')].map((cluster) => {
  ...
  return {
    overlaps,
    primaryOverflow: primary.scrollWidth > primary.clientWidth + 1,
...
const mobileRecordState = await page.evaluate(() => {
  ...
  const tabs = drawer.querySelector('.record-tabs');
  const close = drawer.querySelector('.mobile-close');
  const closeRect = close.getBoundingClientRect();
  const annotationActions = drawer.querySelector('.annotation-actions');
  return {
...
fs.writeFileSync(path.join(outputRoot, 'polish-wave4-browser-audit.md'), [
  ...
  '- Status toast anchored inside the lower-right safe area: PASS',
